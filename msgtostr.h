...
const char *CWinMsgToStr(UINT msg)
{
    for(int i = 0; i < sizeof(MsgToStrMap) / sizeof(*MsgToStrMap); i++)
        if(MsgToStrMap[i].id == msg)
            return MsgToStrMap[i].name;
    return NULL;
}

void CPrintWinMsg(UINT msg, WPARAM wParam, LPARAM lParam)
{
    int n;
    n = printf("%s[%u]", CWinMsgToStr(msg), msg);
    while(n < 30)
    {
        printf(" ");
        n++;
    }
    n = printf("%#I64x[%I64d]", wParam, wParam);
    while(n < 22)
    {
        printf(" ");
        n++;
    }
    n = printf("%#I64x[%I64d]", lParam, lParam);
    printf("\n");
}
